...
const WORKER_SYSTEM = [
  ...
  '',
  '아래 권위 출처를 web_search 로 **반드시 먼저** 조회하세요. 그다음 이 출처들에서 얻은 후보를',
  '추가 web_search 로 **교차검증·보강**하세요. 권위 출처 조회만으로 끝내지 말고, 누락이 의심되면',
  '항상 추가 검색을 실행하세요.',
  '',
  sourcesPromptBlock(),
  '',
  ...
  '점유하는 행사)도 빠짐없이 포함하세요. 종합 일정(마라톤GO·KorMarathon)에서 그 달 서울 대회를',
  '전수 열거한 뒤, 각 대회가 도로 교통통제를 수반하는지 확인해 해당하면 포함하세요.',
  '',
